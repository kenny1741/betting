...
const ALL_LEAGUES = LEAGUES.map(l => ({ ...l, source: "fdo" as const }));
...
export default function AdminPage() {
  ...
  async function runSyncAll(type: SyncType) {
    ...
    if (type === "live") {
      setSteps([{ league: "All Leagues", status: "running" }]);
    ...
    // Fixtures: one call per league, sequentially
    setSteps(ALL_LEAGUES.map(l => ({ league: l.name, status: "pending" })));
    ...
    for (let i = 0; i < ALL_LEAGUES.length; i++) {
      ...
      // FDO rate limit — pause between leagues
      if (i < ALL_LEAGUES.length - 1) await new Promise(r => setTimeout(r, 1200));
    }
    ...
    <div style={s.container}>
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ fontSize: "0.85rem", color: "rgba(255,255,255,0.35)", marginTop: "0.4rem", lineHeight: 1.6 }}>
          Syncs one league at a time to stay within rate limits.
          Covers {LEAGUES.length} leagues via football-data.org.
        </p>
      </div>
            ...
            {steps.map((step, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: 12, fontFamily: "JetBrains Mono, monospace" }}>
                  {step.status === "running" && <span style={{ color: "var(--amber)" }}>syncing…</span>}
                  {step.status === "done"    && <span style={{ color: "var(--green)" }}>{step.synced ?? 0} records</span>}
                  {step.status === "error"   && (
      ...
      <div style={{ ...s.card, marginBottom: "1.5rem" }}>
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>
          Configured Leagues ({LEAGUES.length} total · football-data.org)
        </div>
...
{
  "crons": [
    // FDO leagues (0-${LEAGUES.length - 1}) — stagger by 5 min to respect rate limits
        ...
        <pre style={{ background: "rgba(0,0,0,0.3)", borderRadius: 8, padding: "0.75rem", fontSize: "0.72rem", color: "rgba(255,255,255,0.55)", overflow: "auto", lineHeight: 1.6 }}>{`// vercel.json
{
  "crons": [
    ...
    // Live updates every 2 min (covers all leagues in one call)
    { "path": "/api/sync?secret=SECRET&type=live", "schedule": "*/
